...
In chapter \@ref(viz1) we introduced ggplot as a way to make figures, and in chapter \@ref(goodfigs) we talked about what makes a good plot. **So now you might be a bit frustrated as you know how to make plots in R, and you know what makes a good plot, but you can't make a good plot in R.** I have two answers:    
...
```{block2, type='rmdwarning'}
Making and critiquing plots is one of my favorite parts of science -- I love it!!!! **But I know it can be a huge time suck**, and we do not want that. Here are  my tips to avoid spending too much time on every figure.   
...
The data on daphnia resistance to cyanobacteria is reported as a proportion, which are often best presented on a scale from zero to one. However a simple plot with `geom_point()` spans the range of the data, and does not go from zero to one. Set `limits` in the [`scale_y_continuous()`](https://ggplot2.tidyverse.org/reference/scale_continuous.html) function to change this
...
There are a bunch of ways to add direct labels. The easiest way is with the   [`annotate()`](https://ggplot2.tidyverse.org/reference/annotate.html)  function, introduced above.

But a slicker way is to use [`geom_text()`](https://ggplot2.tidyverse.org/reference/geom_text.html) and summarize your data on the fly (we introduce this in the next chapter).

```{r direct1, fig.cap  = 'Direct labelling', fig.height=3, fig.width=3, message=FALSE, warning=FALSE}
ggplot(iris, aes(x = Sepal.Length, y = Petal.Length, color = Species)) +
  geom_point(alpha = .2)+
  geom_text(data = . %>%
              group_by(Species) %>%
              summarise_at(c("Sepal.Length", "Petal.Length"), mean),
              aes(label = Species), fontface = "bold")+
...
In chapter \@ref(goodfigs) we discussed the advantages of universal design - so how do we achieve this in R?  
