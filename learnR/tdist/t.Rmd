...
# bootstrap
boots <- replicate(n = n_reps, simplify = FALSE,
    expr = hyenas %>%
...
There are complicated ways to do this - but we can simply choose to randomly swap the sign or not as an easy hack. 


```{r perm, exercise=TRUE, eval = FALSE}
n_reps <- 1000

perms  <- replicate(n = n_reps, simplify = FALSE, 
                    expr = hyenas    %>%
  select(subord_minus_dom) %>%
  mutate(swap_sign = sample(c(1,-1),size = n(),replace = TRUE),
         subord_minus_dom = subord_minus_dom * swap_sign)%>%
  summarise(mean_diff = mean(subord_minus_dom)) ) %>%
  bind_rows()
...
As discussed in the chapter, we can use a binomial test, where the "number of successes" is the number of values greater than the null, the number of trials is the number of observations (in this case pairs), and the null probability of success is 0.5. 
