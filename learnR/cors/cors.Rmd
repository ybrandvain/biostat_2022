...
```{r boot, message=FALSE, warning=FALSE, echo=TRUE, eval = FALSE}
n_reps <- 10000

# bootstrap
penguin_boot_dist  <- replicate(n = __, simplify = FALSE, 
      expr = my_penguins %>%
        slice_sample(prop = __, replace = __) %>%
...
observed_cor    <- my_penguins %>%
  summarise(___(__,___)) %>%
  pull()

# permute
perm_dist <- replicate(n = n_reps,simplify = FALSE,
  expr = my_penguins %>%
    mutate(flipper_length = ___(____, replace = ___)) %>%
