...
firefly     <- read_csv(firefly_file) %>% # load data
  mutate(id = 1:n(),
         id = factor(id))   # add an id (for shits  -- this isnt necessary]
```
...
First, let's use the [`slice_sample()`](https://dplyr.tidyverse.org/reference/slice.html) function to generate a sample of the same size. You will notice that this code is very similar to what we wrote for sampling (Section \@ref(onesample)) except that we     
...
resampled_fireflies <- firefly  %>%         
  slice_sample(prop = 1, replace = TRUE) # resample the data with replacement
```
...
Let's do this many (10,000) times with the [`replicate()`](https://www.rdocumentation.org/packages/Rpdb/versions/2.2/topics/replicate) function, much like we did  Section \@ref(repsamp) -- except we set **`prop = 1` and `replace = TRUE`** <span style="color: lightgrey;"> (rather than `size = sample.size` - were is `sample.size`$<$ `population.size`, and `replace =  FALSE`)</span>. We then visualize this bootstrapped sampling distribution as a histogram.
...
firefly_boot <- replicate(n = n.reps, simplify = FALSE, # tell R to do this a bunch, and to keep each result as its own tibble
    expr = slice_sample(firefly, prop =  1, replace = TRUE) %>%    # same code as above
    summarise(boot_mean_flash = mean(flash)))        %>%           # calculate proportion with mean()
  bind_rows()      

# recall we're grouped by replicate when we come out of rep_sample_n()

boot_dist_plot <- ggplot(firefly_boot, aes(x = boot_mean_flash)) +   # assign plot to a variable
...
firefly_boot_SE <- firefly_boot %>% 
  summarise(SE = sd(boot_mean_flash))
...
firefly_boot_CI <- firefly_boot %>% 
  summarise(CI         = c("lower 95", 'upper 95'), # add a column to note which CIs we're calcualting
            mean_flash = quantile(boot_mean_flash ,probs = c(.025,.975)))  # boundaries separating the middle 95% from the rest of our bootstrap replicates. 
...
Here, I show two commonly used packages -- called [`infer`](https://infer.netlify.app/)   and `[mosaic`](https://github.com/ProjectMOSAIC/mosaic). Both packages do way more than bootstrapping, and have been developed in concert with statistics courses. 


#### The `infer` package   

Check out details [here](https://infer.netlify.app/)   

```{r}
boot_firefly_mean_flash <-  firefly %>%
    specify(response = flash) %>%
    generate(reps = 10000, type = "bootstrap") %>%
    calculate(stat = "mean")

boot_firefly_mean_flash %>% 
  summarise(CI         = c("lower 95", 'upper 95'), # add a column to note which CIs we're calcualting
            mean_flash = quantile(stat ,probs = c(.025,.975))) 
```

#### The `mosiac` package   

Check out details [here](https://github.com/ProjectMOSAIC/mosaic)   

```{r}
library(mosaic)
# bootstrap
boot_firefly_mean_flash <- mosaic::do(10000) *  mean( ~ flash, data = resample(firefly)) 
# find conf interval
confint(boot_firefly_mean_flash, level = 0.95, method = "quantile")
```
