...
```{block2, type='rmdnote'}
These notes accompany portions of Chapter 4  --- *Estimating with Uncertainty* --- of our textbook, which we revisit in the next Section.   **The reading below is required,**  @whitlock2020 is  not.  
...
write_csv(coa,file = "data/collegeOfAtlantic.csv")
```
...
coa_sample <- slice_sample(coa, n = 30, replace = FALSE) 
n_faculty  <- coa_sample %>%
  summarise(sum(role == "faculty"))  %>%  # counts number of faculty. rember TRUE = 1 
  pull() 
```
...
#### Building a sampling distribution by repeatedly sampling  {#repsamp}

```{r, eval=FALSE, echo=FALSE}
coa_dist<- lapply(500:1,function(i){
  prop <- slice_sample(coa, n = 30) %>% 
    filter(role == "faculty") %>% 
    nrow() / 30
...
```{r , echo=FALSE}
include_graphics("images/moresampling.gif" ) 
```




```{r coadist, fig.cap = 'shows the different estimates of the proportion of faculty at College of the Atlantic with an unbiased an independent sample of size 30 -- building a histogram of sample estimates if we were to sample a bunch. This histogram is known as the **sampling distribution**.',echo=FALSE, out.width='40%',out.extra='style="float:right; padding:10px"'}
include_graphics("images/coasampling.gif")
```


We can make a sampling distribution by resampling from a population by replicating our code a bunch of times with the [`replicate()`](https://www.rdocumentation.org/packages/Rpdb/versions/2.2/topics/replicate) function. For mundane formatting reasons, we need to set the argument `simplify = TRUE` inside of [`replicate()`](https://www.rdocumentation.org/packages/Rpdb/versions/2.2/topics/replicate) and pipe this output to the [`bind_rows()`](https://dplyr.tidyverse.org/reference/bind.html) function. The code below shows how we can generate the data for Figure  \@ref(fig:coadist).     *NOTE:* A slicker way to do this would be to write your own function to do all of this, but that is beyond our immediate goals. 
...
**The take home message here is** whenever you see some exceptional claim be sure to look into the sample size, and measures of uncertainty, which we discuss in next chapter. For a nice discussion check out this optional reading, [The most dangerous equation](https://www.americanscientist.org/article/the-most-dangerous-equation) [@wainer2007].  
...
```{r, echo=FALSE}
include_app('https://brandvain.shinyapps.io/sampling/', height = '800px')
```
...
Many of the concepts discussed in this chapter were first introduced in Section  \@ref(intro) and have been revisited in greater detail in this chapter. 
...
[`slice_sample(.data, ..., n, prop, weight_by = NULL, replace = FALSE)`](https://dplyr.tidyverse.org/reference/slice.html):  Generate a sample of size `n`, from a tibble `.data`, with (`replace = TRUE`) or without (`replacement = FALSE`) replacement. All arguments are the same as in `sample()` except `weight` replaces `prob`, and `.data` replaces `x`. `slice_sample()` is a function in the `dplyr` package, which is loaded with tidyverse.     
```
