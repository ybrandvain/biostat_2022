...
<span style="color: Blue;font-size:22px;">   Motivating scenarios:  </span>  <span style="color: Black;font-size:18px;">   We  want to understand what's so special about the normal distribution -- what are its properties? Why do we see it so often? Why d0 we use it so often? What is it good for?....  </span>
...
```{r norm1, fig.cap = "A normal distribution with a mean of zero and standard deviation of 0.2.", fig.height=2,fig.width=4, echo=FALSE}
...
We efficiently describe a normally distributed random variable as $X \sim  N(\mu,\sigma^2)$, where $\sim$ means *is distributed* (e.g. $X$ is normally distributed with a mean \mu and variance \sigma^2). So, the mathematical notation to say $X$ is normally distributed with mean $\mu = 0.5$ and variance $\sigma^2 = 0.1^2$ is $X \sim N(0.5,0.01)$.
