...
`R` is a computer program, which is the go to language for most statisticians and data scientists (although some prefer [python](https://www.python.org/) or [julia](https://julialang.org/)). R makes it possible to conduct complex statistical analyses, make nice figures, and write this book, all in one environment.
...
2. Then **download/update RStudio** [from
here](https://rstudio.com/products/rstudio/download/#download). Be sure to select the free version (RStudio Desktop, Open Source License), and as above, be sure to download the version compatible with your computer. If you installed RStudio a while ago, be sure you’re using RStudio version 1.3.0 or above.  
...
include_graphics("images/Rstudio.jpeg")
```
...
- 'Is four divided by four less than or equal to one?' like this `4/4 <= 1` (note a less than sign followed by the equals sign), which returns `TRUE`.  
- 'Is four divided by four greater than or equal to one?' like this `4/4 >= 1` (note a greater then sign, then an equals sign), which returns `TRUE`.   
...
Complete the quiz below, a nearly  identical quiz will be on [canvas](https://canvas.umn.edu/courses/284485/quizzes/558863).
