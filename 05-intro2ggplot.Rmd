...
```{block2, type='rmdnote'}
These notes accompany portions of Chapter 2 — Displaying Data — of our textbook, which we revisit later. The reading below is required, @whitlock2020 is not.
```
...
There is no external reading for this chapter, but watch the embedded vidoes, and complete all embedded learnR excercises. Then go to canvas to fill out the evaluation.  You will need to make three very different types of plots from the `mpg` data. 
```
...
## An intro to today's data   


So, let’s do this with a data set [msleep](https://ggplot2.tidyverse.org/reference/msleep.html) (click link for more info) built in to tidyverse, which contains interesting information about a bunch of mammals. One question we might have is - how much variability is there in mammal brain size, and what explains this variation? Let’s look to find out.
...
Here are the data to get a sense of it.   
...
```{block2, type='rmdnote'}
**Adding things to a `ggplot`**. You see that we keep adding to our plots with a `+` sign. I often accidently use the pipe operator `%>%` sign used to pipe results from one procedure to the next in `dplyr` by mistake. R will gently notify you when you make this common error.   
```


### Adding geom layers    

`geoms` explain to R what we want to see: points, lines, histograms etc... In a future chapter, we will see how to add data summaries and trendlines. As we discuss below, a histogram is a great way to visualize variability, so lets add that as a `geom`.
...
```{r second-fig, fig.width=8, fig.cap = 'Our first facetted ggplot!', fig.height=3, warning=FALSE}
ggplot(data = msleep, aes(x = log10_brainwt))   +
  geom_histogram(bins =10, color = "white") +
...
Above, we made  our final faceted figure (\@ref(fig:first-fig)) by copying what we coded in our histogram figure (\@ref(fig:first-fig)) and adding the  `facet_wrap()` function. Alternatively, we can recreate Fig. \@ref(fig:second-fig) by saving the plot as we go and adding on as follows:

```{r}
plot_one <- ggplot(data = msleep, aes(x = log10_brainwt)) +
  geom_histogram(bins =10, color = "white")                 
...
In a **histogram**, *each value on the x represents some interval of values of our categorical variable* (in this case, we had 10 bins, but we could have, for example, looked at sleep in one hour with `binwidth = 1`), while *y-values show how many observations correspond to an interval on the x*. 
...
Sometime we want points connected by lines (i.e. a lineplot). Lineplots are most usefull when we are looking at trends over time (e.g. when the x-axis is time).  In such cases, [`geom_line()`](https://ggplot2.tidyverse.org/reference/geom_path) is here for us! Below I first modify the [`lynx`](https://www.rdocumentation.org/packages/datasets/versions/3.6.2/topics/lynx) dataset -- which describes the number of lynx trapped in Canada every year from 1821 to 1934 -- to make it a tibble (don't worry about this code).
...
We can now plot these data as a lineplot 

```{r, fig.cap="A lineplot for the lynx dataset", fig.width=5, warning=FALSE, fig.height=2}
ggplot(canadian_lynx, aes(x  = year, y = lynx_trapped))+
  geom_line()
```
```{r, fig.cap="A scatterplot that should be a lineplot for the lynx dataset", fig.width=5, warning=FALSE, fig.height=2}
ggplot(canadian_lynx, aes(x  = year, y = lynx_trapped))+
  geom_point()
```


As you can see, the lineplot more clearly show the trends than does the scatterplot. 
...
Fill out the quiz on canvas, which is very similar to the one below.
