...
```{block2, type='rmdnote'}
**Adding things to a `ggplot`**. You see that we keep adding to our plots with a `+` sign. I often accidently use the pipe operator `%>%` sign used to pipe results from one procdure to the next in `dplyr` by mistake. R will gently notify you when you make this common error.   
```
