...
```{r, echo=FALSE}
include_graphics("images/cov.gif")
```
...
```{r, echo=FALSE}
DT::datatable(crickets_one_resample,
...
From this resampled data set, we estimate a correlation of `r round(pull(crickets_one_boot),digits = 3)` -- as compared to the correlation of `r round(cor(crickets)[1,2], digits = 3)` observed in the real data.
...
```{r, echo=FALSE}
DT::datatable(head(crickets_boots %>% mutate_all(round,  digits = 3), n = 1000),
              options = list(autoWidth = TRUE,pageLength = 5, lengthMenu = c(5, 25, 50)
))
```
...
**So we find find a negative correlation between sperm motility and lyzosyme activity, with an estimated correlation coefficient of `r round(cor(crickets)[1,2], digits = 3)`, and Standard error of `r round(crickets_SE_cor, digits  = 4)`, and a 95% confidence interval between  `r filter(crickets_CI_cor, bound == "lower")%>% pull(CI) %>% round(digits = 3)` and `r filter(crickets_CI_cor, bound == "upper")%>% pull(CI) %>% round(digits = 3)`.**
...
So, our permutation yielded a correlation of `r round(pull(crickets_one_perm_cor) ,digits = 3)` which is 
`r case_when(pull(crickets_one_perm_cor)  > cor(crickets)[1,2] ~ "greater than", pull(crickets_one_perm_cor)  < cor(crickets)[1,2] ~ "less than", pull(crickets_one_perm_cor)  == cor(crickets)[1,2] ~ "equal to")` to correlation of `r  round(cor(crickets)[1,2],digits = 3)` found in the data. We can't make much of this one sample, so we need to build a sampling distribution under the null to see if our result is indeed an unusual outcome of the null model. 

####  ***Permute and summarize the data many times to generate a sampling distribution for the null model***  
...
```{r, echo=FALSE}
DT::datatable(head(crickets_perms %>% mutate(cor_vals = round(cor_vals, digits = 3), actual_cor = round(actual_cor, digits = 3)), n = 1000),
              options = list(autoWidth = TRUE,pageLength = 5, lengthMenu = c(5, 25, 50)
))
```
...
include_graphics("images/broom_package.png")
```
